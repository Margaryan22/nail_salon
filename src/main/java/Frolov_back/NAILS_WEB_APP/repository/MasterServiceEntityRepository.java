...
public interface MasterServiceEntityRepository extends BaseRepository<MasterServiceEntity, MasterServiceId> {
    ...
    List<MasterServiceEntity> findByService(NailService service);

    // Найти конкретную связь мастер-услуга
    Optional<MasterServiceEntity> findByMasterAndService(SystemUser master, NailService service);
    ...
    List<MasterServiceEntity> findByServiceOrderByMasterPriceAsc(NailService service);

    // Удалить связь мастер-услуга
    void deleteByMasterAndService(SystemUser master, NailService service);

    // Проверить существует ли связь
    boolean existsByMasterAndService(SystemUser master, NailService service);
